...
<body>
    <div>
        ...
        <h4 class="mt-2 font-bold text-2xl text-slate-700">Điện thoại</h4>
        <ul class="p-0 pt-2 m-0 flex flex-wrap justify-evenly">
            <?php 
                ...
                products.thumbnail as thumbnail, products.discount as discount from products join categories on products.categoryId = categories.id where name like 'phone' order by createAt desc";
                // $appleAuthSql = "SELECT * FROM productdetails WHERE LOWER(brand) LIKE 'apple' LIMIT 20";
                $phoneQuery= mysqli_query($conn, $phoneSql);
                while($rowP=mysqli_fetch_array($phoneQuery)){
            ?>
                ...
                <li class="itemP list-none p-2 m-px my-1.5 border rounded shadow-md relative hover:shadow">
                    ...
                    <p><?php echo number_format($rowP['price']*(1-$rowP['discount'])); ?> ₫</p>
                <span class="text-sm line-through" <?php echo ($rowP['discount']>0)?"":"hidden"?>><?php echo number_format($rowP['price']);?> ₫</span>
                <span class="text-xs font-semibold" <?php echo ($rowP['discount']>0)?"":"hidden"?>>Giảm <?php echo $rowP['discount']*100?>%</span>
